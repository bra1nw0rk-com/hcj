...
export default class WEBFS {
	static api(url, json, callback) {
		...
		return new Promise((resolve, reject) => {
			fetch(url, {
				...
				.then((data) => {
					result = data; // Store the response data in `result`
					if (typeof callback === 'function') {
						callback(result); // Call the callback if defined
					}
				...
				.catch((error) => {
					...
					if (typeof callback === 'function') {
						callback(result); // Call the callback with the error result
					}
					resolve(result); // Resolve with the error result
				});
		});
	}
	...
	static getContentType(ext) {
		...
		let _ext = ext.replace(/\?.*$/, '').split('.').pop();
