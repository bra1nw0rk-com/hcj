...
export default class Box extends HTMLObject {
	...
	get icon(){
		...
		return {
			info(){
				_this.#icon = `<i class="fa fa-info icon" aria-hidden="true"></i>`
				_this.#head.prepend($(_this.#icon));
			},
			error(){
				_this.#icon = $(`<i class="fa fa-times-circle icon" aria-hidden="true"></i>`);
				_this.#head.prepend($(_this.#icon));

			},
			settings(){
				_this.#icon = `<i class="fa fa-cogs icon" aria-hidden="true"></i>`;
				_this.#head.prepend($(_this.#icon));
			},
			set(icon){
				_this.#icon = `<i class="fa ${icon} icon" aria-hidden="true"></i>`;
				_this.#head.prepend($(_this.#icon));
			}
	...
	close(callback){
		WS.ui.modal.close(`#${this.id}`, callback);
